...
QList<QVariantMap> SqlService::getItems(QString table, int pageFrom, int pageNum)
{
...
bool SqlService::initDb(QString dbname, QString table, QMap<QString, QString> map)
{
...
QMap<QString, QString> SqlService::getTableInfo(QString table)
{
    ...
    if (m_sqlQuery.exec()) {
        ...
        while (m_sqlQuery.next()) {
            ...
            result.insert(m_sqlQuery.value(1).toString(), m_sqlQuery.value(2).toString());
        }
    }
    return result;
}
