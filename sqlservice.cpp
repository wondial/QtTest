...
bool SqlService::exec() { return m_sqlQuery.exec(); }
...
bool SqlService::createTable(QString table, QMap<QString, QString> map) {
  ...
  const auto IF_EXIST_TABLE_SQL =
      QLatin1String(
          R"(select * from sqlite_master where type='table' and name='%1')")
          .arg(table);
  m_sqlQuery.exec(IF_EXIST_TABLE_SQL);
  if (m_sqlQuery.next()) {
    setLastError(QString("( 已存在 %1 表! )").arg(table));
    return false;
  }
...
bool SqlService::deleteItem(QString table, QVariantMap map) {
  ...
  while (mi.hasNext()) {
    ...
    if (mi.hasNext())
      DELETE_SQL +=
          QString("%1 = '%2', ").arg(mi.key()).arg(mi.value().toString());
    else
      DELETE_SQL +=
          QString("%1 = '%2' ").arg(mi.key()).arg(mi.value().toString());
  }
...
QList<QVariantMap> SqlService::getItems(QString table, int pageFrom,
                                        int pageNum) {
  QList<QVariantMap> list;
  this->exec(QString("select * from %1").arg(table));
...
QList<QVariantMap> SqlService::getItems(QString table) {
  QList<QVariantMap> list;
  this->exec(QString("select * from %1").arg(table));
  if (m_sqlQuery.record().count() < 0) {
    setLastError("getValues error![The number of pages is beyond the limit]");
    return list;
  }

  qDebug() << "Number of columns: " << m_sqlQuery.record().count();
